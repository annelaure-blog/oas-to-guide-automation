...
class FieldInfo:
    ...
    def render_description(self) -> str:
        ...
        parts = [self.description.strip() or "No description provided.",
...
def generate_markdown(context: OperationContext) -> str:
    ...
    endpoint_table = textwrap.dedent(
        ...
        | :--- | :------- | :---------- |
        | **{context.method.upper()}** | `{context.path}` | {context.description or context.summary or 'No description provided.'} |
        """
    ).strip()
...
def render_fields(fields: Sequence[FieldInfo]) -> str:
    ...
    rows = [f"| {f.attribute} | {f.data_type} | {f.render_description().replace('\n', '<br>')} |" for f in fields]
    return "\n".join([header, *rows])
...
def render_code_samples(context: OperationContext) -> str:
    ...
    python_lines = [
        ...
        f"payload = json.dumps({payload_compact}) if {has_body} else None",
        "",
        "headers = {",
        headers_block,
        "}",
