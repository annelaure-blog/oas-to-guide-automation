...
def render_code_samples(context: OperationContext) -> str:
    ...
    payload_json = (
        json.dumps(payload_data, indent=4, sort_keys=True) if payload_data is not None else ""
    )
    payload_compact = json.dumps(payload_data, sort_keys=True) if payload_data is not None else ""
...
def render_response_example(responses: Mapping[str, Any]) -> str:
    ...
    return f"```json\n{json.dumps(example_payload, indent=4, sort_keys=True)}\n```"
...
def build_example_from_schema(schema: Optional[Mapping[str, Any]]) -> Any:
    ...
    if schema_type == "object" or ("properties" in schema and not schema_type):
        ...
        for prop_name, prop_schema in sorted((schema.get("properties") or {}).items()):
